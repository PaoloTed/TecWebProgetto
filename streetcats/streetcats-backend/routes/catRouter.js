...
import { enforceAuthentication, optionalAuthentication, requireOwnerOrAdmin } from "../middleware/authorization.js";
...
// ========== ROTTE PROTETTE (POST, PUT, DELETE) ==========
...
catRouter.put("/cats/:id", enforceAuthentication, async (req, res, next) => {
  try {
    ...
    const cat = await CatController.findById(catId);
    ...
    const updatedCat = await CatController.updateCat(catId, req.body);
    res.json(updatedCat);
  } catch (err) {
    next(err);
  }
...
catRouter.delete("/cats/:id", enforceAuthentication, async (req, res, next) => {
  try {
    ...
    const deletedCat = await CatController.deleteCat(catId);
...
/**
 * POST /cats/:id/comments - Aggiungi commento a un gatto (richiede autenticazione)
 */
catRouter.post("/cats/:id/comments", enforceAuthentication, async (req, res, next) => {
  try {
    ...
    if (err.message === 'Gatto non trovato') {
